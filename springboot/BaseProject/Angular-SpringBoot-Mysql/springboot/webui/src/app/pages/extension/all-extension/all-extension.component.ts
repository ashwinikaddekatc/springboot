...
export class AllExtensionComponent implements OnInit {
  ...
  toggle(id: number) {
    ...
    this.extensionService.getById(id).subscribe(ext => {
      ...
      //console.log('extField ', this.extField);
      console.log('checked ', this.checked);
      console.log(this.extField)
    });
    console.log(this.extField.isActive);
    if (this.extField.isActive == false) {
      this.extField.isActive = true
      this.extensionService.update(id, this.extField).subscribe(updatedext => {
        console.log(`field id is ${id} `);
        console.log(updatedext);
      })
    }
    else if (this.extField.isActive == true) {
      this.extField.isActive = false
      this.extensionService.update(id, this.extField).subscribe(updatedext => {
        console.log(`field id is ${id} `);
        console.log(updatedext);
      })
